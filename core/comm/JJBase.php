...
class JJBase
{
	...
	{
		  if(!isset(self::$instance['view']))
		{
			$filename=COREPATH.'/comm/View.php';
			require_once($filename);
   ...
   /**
	...
	public function getContrNameExceptContr()
	{
		if($_GET['c']==''||!isset($_GET['c']))return 'Index';
		$contrName=trim(ucfirst($_GET['c']));
		if (false === preg_match('/^[a-z0-9_-]+$/i', $contrName))throw new JJException('无效的控制器'.$contrName); 
		return $contrName;
	}
	/**
